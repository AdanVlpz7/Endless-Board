...
public class CameraController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            bg1.position = SetPos(bg1TargetPos, bg1.position.x, bg2.position.y + size - 400, bg1.position.z);
            Switching();
        }
        if(transform.position.y <= bg1.position.y)
        {
            bg2.position = SetPos(bg2TargetPos, bg2.position.x, bg1.position.y - size + 400, bg2.position.z);
            Switching();
        }
    }
